...
- **Instrument from day one.** Track the numbers that reveal whether the process is improving, such as cycle time, throughput, and manual-override frequency, so you measure the process, not just monitor uptime. Techniques such as [process mining](https://www.tf-pm.org/) turn that event data into a picture of how the process actually runs.
